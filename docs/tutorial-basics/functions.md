...
Perhaps the most used function in the framework. This functions returns the players data of the current source which, since its used client side, is automatically the client or player. It can be used with modifiers on the end starting with a "." (period).
...
Current types: primary(default), success, error, police, ambulance - An unlimited amount of types can be made in qb-core/html/app.js
...
Returns vehicle game pool (for backwards compatiblity) - Not worth using
```lua
...
```
### Example exports QBCore.Functions.GetCoreObject
You can simply import QBCore by calling the export below.
```lua
local QBCore = exports['qb-core']:GetCoreObject()
```

### QBCore.Functions.GetPlayers
Returns active players in scope (for backwards compatiblity) - Not worth using
```lua
...
Returns filtered ped game pool
```lua
...
Returns a filtered closest ped to the client
```lua
...
Returns the closest vehicle to the client
```lua
...
Returns the closest object to the client
```lua
